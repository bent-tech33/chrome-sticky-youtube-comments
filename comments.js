...
docReady(function() {
	...
	function updteWidth(){
		var chapters = document.querySelectorAll(".ytp-chapters-container .ytp-chapter-hover-container");
		for (var i=0, max=chapters.length; i < max; i++) {
			var tempWidth = parseInt(chapters[i].style.width);
			chapters[i].style.width = (tempWidth - (tempWidth * 0.25))+'px';
			//-25% of what it is to compensate for the total browser width loss
			console.log('old width: '+tempWidth+' new width:'+chapters[i].style.width );
		}
	}
